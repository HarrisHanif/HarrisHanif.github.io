...
def eval_genomes(genomes, config, checkpointer, p):
    ...
    for genome_id, genome in genomes:
        ...
        pl.addanalyzer(bt.analyzers.Returns, _name='returns')
        ...
        unprofitable_consecutive_trades = 0
        ...
        if profitable_consecutive_trades > max_consecutive_trades:
            genome.fitness = sharpe_ratio - (trade_penalty_factor * (profitable_consecutive_trades - max_consecutive_trades) * 0.01)

        # Penalize for too many consecutive unprofitable trades
        elif unprofitable_consecutive_trades > max_consecutive_trades:
            genome.fitness = sharpe_ratio - (trade_penalty_factor * (unprofitable_consecutive_trades - max_consecutive_trades) * 0.01)

        # Reward for maximizing Sharpe Ratio
        else:
